...
import com.example.ballen.backend.data.Availability;
...
public class ProductForm extends Div {
    ...
    public ProductForm(SampleCrudLogic sampleCrudLogic) {
        ...
        availability.setItemLabelGenerator(a -> getTranslation(a.getKey()));
        availability.setAllowCustomValue(false);
        content.add(availability);

        category = new CheckboxGroup<>();
        category.setId("category");
        category.getContent().getStyle().set("flex-direction", "column").set("margin", "0");
        ...
        delete.setWidth("100%");
        delete.getElement().getThemeList().addAll(Arrays.asList("error", "primary"));
